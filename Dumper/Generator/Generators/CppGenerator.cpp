...
int32 CppGenerator::FindFreeOffset(const std::vector<PredefinedMember>& Properties, int32 Size, int32 Alignment)
{
	std::vector<std::pair<int32, int32>> Ranges;

	for (const auto& Property : Properties)
	{
		if (Property.bIsStatic || Property.bIsZeroSizeMember)
			continue;

		int32 Start = Property.Offset;
		int32 End   = Property.Offset + Property.Size * Property.ArrayDim;
	...
	// Align the offset after the last member.
	return (Offset + Alignment - 1) & ~(Alignment - 1);
}

std::string CppGenerator::MakeMemberString(
    const std::string& Type,
    const std::string& Name,
    std::string&& Comment)
{
	const auto& Settings = GSettings.CppGenerator;

	if (Settings.UseMemberCustomSpacing)
	{
		const int TypeWidth = Settings.MemberTypeColumnWidth;
		const int NameWidth = Settings.MemberNameColumnWidth;

		int NumSpacesToComment;

		if (Type.length() < static_cast<size_t>(TypeWidth))
		{
			NumSpacesToComment = NameWidth;
		}
		else if ((Type.length() + Name.length()) > static_cast<size_t>(TypeWidth + NameWidth))
		{
			NumSpacesToComment = 1;
		}
		else
		{
			NumSpacesToComment = NameWidth - (Type.length() - TypeWidth);
		}
	...
	return fmt::format("\t{} {}; // {}\n", Type, Name, std::move(Comment));
}


std::string CppGenerator::MakeEnumMemberString(const std::string& Name, const std::string& Value, const std::string& Underlyingtype, bool bIsOutOfRange)
{
	const auto& Settings = GSettings.CppGenerator;
	...
	if (Settings.UseMemberCustomSpacing)
	{
		if (bIsOutOfRange)
		{
			return fmt::format("\t// {:{}} = {} // (out of range for {}),\n", Name, Settings.MemberNameColumnWidth, Value, Underlyingtype);
		}
		return fmt::format("\t{:{}} = {},\n", Name, Settings.MemberNameColumnWidth, Value);
	}

	if (bIsOutOfRange)
	{
		return fmt::format("\t// {} = {} // (out of range for {}),\n", Name, Value, Underlyingtype);
	}
	return fmt::format("\t{} = {},\n", Name, Value);
}
...
std::string CppGenerator::GenerateMembers(const StructWrapper& Struct, const MemberManager& Members, int32 SuperSize, int32 SuperLastMemberEnd, int32 SuperAlign, int32 PackageIndex)
{
	...
	for (const PropertyWrapper& Member : Members.IterateMembers())
	{
		...
		if (CurrentPropertyEnd > PrevPropertyEnd && bLastPropertyWasBitField && bIsBitField && uint64(PrevBitPropertyEndBit) < PrevNumBitsInUnderlayingType && !bIsUnion)
		{
	...
	/*
	 * A struct with bHasReusedTrailingPadding is expected to let a derived class place its own
	 * first member inside its trailing bytes (the same layout a real empty base class gets via the
	 * compiler's empty-base-optimization/tail-padding reuse). Materializing an explicit pad member
	 * here would occupy that space with real data, preventing that reuse and shifting a derived
	 * class's members to the wrong offset.
	 *
	 * That reuse is only reliable when PrevPropertyEnd == 0, ie. every ancestor up to this point
	 * (this struct included) is genuinely empty with nothing real anywhere in the chain -- the
	 * classic empty-base-optimization case, where the compiler contributes zero bytes regardless of
	 * alignment. The moment some ancestor further up actually has real members (PrevPropertyEnd != 0),
	 * this struct being empty does NOT let a descendant's members start any earlier than that real
	 * ancestor's own end: plain, non-polymorphic types get no further tail-padding reuse beyond that
	 * point, so a gap here has to be filled explicitly or a descendant's members land one-or-more
	 * bytes earlier than the engine reports (confirmed empirically -- a real build placed a member
	 * exactly at the non-empty ancestor's unaligned end, not at this struct's own, larger, guessed
	 * "unaligned size"). Likewise, if the gap itself exceeds this struct's own alignment, bare
	 * alignment alone can never reach Struct.GetUnalignedSize() regardless of what feeds PrevPropertyEnd.
	 */
	if (!Struct.HasReusedTrailingPadding() || Struct.GetUnalignedSize() > Struct.GetAlignment() || PrevPropertyEnd != 0)
	{
...
{
	const auto& CppSettings = GSettings.CppGenerator;
	...
	// Function declaration and inline-body generation
	{
		if (!Func.IsPredefined())
		{
			const auto UnrealFunc = Func.GetUnrealFunction();
			uintptr_t FuncOffset  = GMemory->GetUnrealModule().AddressToOffset(reinterpret_cast<uintptr_t>(UnrealFunc.GetExecFunction()));

			if (Func.HasFunctionFlag(EFunctionFlags::Native) & 1)
			{
				InHeaderFunctionText += fmt::format("\n\t// {}\n\t// [{}]\n\t// NumParams({}) | ParamSize(0x{:X}) | NativeExecOffset(0x{:X})\n", UnrealFunc.GetFullName(), UnrealFunc.StringifyFlags(), UnrealFunc.GetNumParams(), UnrealFunc.GetParamSize(), FuncOffset);
			}
			else if (Func.HasFunctionFlag(EFunctionFlags::BlueprintEvent) & 1)
			{
				InHeaderFunctionText += fmt::format("\n\t// {}\n\t// [{}]\n\t// NumParams({}) | ParamSize(0x{:X}) | UObject::ProcessInternal(0x{:X})\n", UnrealFunc.GetFullName(), UnrealFunc.StringifyFlags(), UnrealFunc.GetNumParams(), UnrealFunc.GetParamSize(), FuncOffset);
			}
			else if (Func.HasFunctionFlag(EFunctionFlags::UbergraphFunction) & 1)
			{
				InHeaderFunctionText += fmt::format("\n\t// {}\n\t// [{}]\n\t// NumParams({}) | ParamSize(0x{:X}) | UObject::ProcessInternal(0x{:X})\n", UnrealFunc.GetFullName(), UnrealFunc.StringifyFlags(), UnrealFunc.GetNumParams(), UnrealFunc.GetParamSize(), FuncOffset);
			}
		}
...
{}
	...
	std::string ParamStructName = GetStructPrefixedName(Func.AsStruct());
	...
	{}{} Parms{{}};
	                                                 ...
	                                                 !CppSettings.ParamNamespaceName.empty() ? fmt::format("{}::", CppSettings.ParamNamespaceName) : "",
	                                                 ParamStructName);
	...
	std::string RetParamName = "ReturnValue";
	...
	{
		if (Settings.UseMemberCustomSpacing)
			ParamDescriptionCommentString += fmt::format("// {:{}} {:{}} ({})\n", PInfo.Type, Settings.MemberTypeColumnWidth, PInfo.Name, Settings.MemberNameColumnWidth, StringifyPropertyFlags(PInfo.PropFlags));
		else
			ParamDescriptionCommentString += fmt::format("// {} {} ({})\n", PInfo.Type, PInfo.Name, StringifyPropertyFlags(PInfo.PropFlags));

		if (PInfo.bIsRetParam)
		{
			RetParamName = PInfo.Name;
			continue;
		}
	...
	if ({0} != nullptr)
		*{0} = std::move(Parms.{0});)",
		...
		{
			if (PInfo.Type.ends_with('*'))
			{
				ParamAssignments += fmt::format("\t*reinterpret_cast<{0}void**>(&Parms.{1}) = reinterpret_cast<{0}void*>({1});\n", PInfo.Type.starts_with("const ") ? "const " : "", PInfo.Name);
			}
			else
			{
	...
	std::string ReturnValueString = fmt::format(R"(
...
{}{}{}
	{}ProcessEvent(Func, {});{}{}{}{}
	                                                 ...
	                                                 bIsConstFunc ? " const" : "",
	                                                 Func.IsStatic() ? "StaticClass()" : Func.IsInInterface() ? "AsUObject()->Class"
	                                                                                                          : "Class",
	                                                 !CppSettings.XORString.empty() ? fmt::format("{}(\"{}\")", CppSettings.XORString, FixedOuterName) : fmt::format("\"{}\"", FixedOuterName),
	                                                 !CppSettings.XORString.empty() ? fmt::format("{}(\"{}\")", CppSettings.XORString, FixedFunctionName) : fmt::format("\"{}\"", FixedFunctionName),
	                                                 bHasParams ? ParamVarCreationString : "",
	                                                 bHasParamsToInit ? ParamAssignments : "",
	                                                 bIsNativeFunc ? StoreFunctionFlagsString : "",
	                                                 Func.IsStatic() ? "GetDefaultObj()->" : Func.IsInInterface() ? "AsUObject()->"
	                                                                                                              : "UObject::",
...
std::string CppGenerator::GenerateFunctions(const StructWrapper& Struct, const MemberManager& Members, const std::string& StructName, StreamType& FunctionFile, StreamType& ParamFile, StreamType& AssertionFile)
{
	...
	std::string NameText = !CppSettings.XORString.empty() ? fmt::format("{}(\"{}\")", CppSettings.XORString, Name) : fmt::format("\"{}\"", Name);
	...
	/* ClassName always uses the short name; prefix matches TCHAR (u"" for char16_t, L"" for wchar_t) */
	const std::string_view litPrefix = InternalSettings::bUseChar16String ? "u" : "L";
	NameText                         = !CppSettings.XORString.empty()
	                                       ? fmt::format("{}({}\"{}\")", CppSettings.XORString, litPrefix, Struct.GetRawName())
	                                       : fmt::format("{}\"{}\"", litPrefix, Struct.GetRawName());
...
void CppGenerator::GenerateStruct(const StructWrapper& Struct, StreamType& StructFile, StreamType& FunctionFile, StreamType& ParamFile, StreamType& AssertionFile, int32 PackageIndex, const std::string& StructNameOverride)
{
	...
	const int32 StructAlignedSize           = Struct.GetSize();
	int32 SuperAlignedSize                  = 0x0;
	int32 SuperUnalignedSize                = 0x0;
	...
	if (bHasValidSuper)
	{
		...
		SuperAlignedSize   = Super.GetSize();
		SuperUnalignedSize = Super.GetUnalignedSize();
...
// {}
// SuperAlignedSize(0x{:04X}) SuperUnalignedSize(0x{:04X}) SuperAlignment(0x{:04X}) SuperLastMemberEnd(0x{:04X})
// bHasReusedTrailingPadding({}) bIsReusingTrailingPaddingFromSuper({})
// StructAlignedSize(0x{:04X}) StructUnalignedSize(0x{:04X}) StructAlignment(0x{:04X})
...
{{
	                          ...
	                          SuperAlignment,
	                          SuperLastMemberEnd,
	                          bHasReusedTrailingPadding,
	                          bIsReusingTrailingPaddingFromSuper,
	                          StructAlignedSize,
	                          Struct.GetUnalignedSize(),
	                          Struct.GetAlignment(),
	                          ...
	                          AlignmentString,
	                          UniqueName,
	                          GSettings.CppGenerator.bAddFinalSpecifier && Struct.IsFinal() ? " final" : "",
	...
	/*
	 * Must agree with GenerateMembers' own decision about whether it will actually emit a trailing
	 * pad member: that's based on the *unaligned* size minus EffectiveSuperSize, not on
	 * StructSizeWithoutSuper (which is derived from the *aligned* StructSize instead). Using the
	 * aligned-size-based gap here let a struct that GenerateMembers would still fill (eg. a 1-byte
	 * gap against an 8-byte-aligned non-empty ancestor) get skipped entirely -- "public:" and the
	 * GenerateMembers() call below never even ran, so no fix inside GenerateMembers could apply.
	 */
	const bool bHasMembers           = Members.HasMembers() || (Struct.GetUnalignedSize() - EffectiveSuperSize) > 0;
	const bool bHasFunctions         = (Members.HasFunctions() && !Struct.IsFunction()) || bHasStaticClass;
	const bool bShouldAddConstructor = !bIsUnion && !Members.HasPredefinedConstructor();

	if (bHasMembers || bHasFunctions || bShouldAddConstructor)
		StructFile << "public:\n";

	if (bShouldAddConstructor)
	{
		StructFile << "\t" << Utils::String::SubstrAfterLast(UniqueName, "::") << "() {}\n\n";
	}

	if (bHasMembers)
	{
		StructFile << GenerateMembers(Struct, Members, EffectiveSuperSize, SuperLastMemberEnd, SuperAlignment, PackageIndex);
	...
	if (bHasFunctions)
	{
		StreamType& FuncParamsAssertionFile = GSettings.Debug.bGenerateAssertionFile ? AssertionFile : ParamFile;
	...
	if (GSettings.Debug.bGenerateAssertionFile)
	{
		if (bIsTemplatedType)
			return;
		...
		StructFile << "#if defined(__clang__) || defined(__GNUC__)\n"
	...
	const char* AssertionNewLineStr = GSettings.Debug.bGenerateAssertionFile ? " \\\n" : "\n";

	if (GSettings.Debug.bGenerateInlineAssertionsForStructSize || GSettings.Debug.bGenerateAssertionFile)
	{
		if (bIsTemplatedType)
			return;
		...
		// Size assertions
		AssertionFile << fmt::format("static_assert(sizeof({}) == 0x{:06X}, \"Wrong size on {}\");{}", UniqueName, StructSize, UniqueName, AssertionNewLineStr);
	}

	if (GSettings.Debug.bGenerateInlineAssertionsForStructMembers || GSettings.Debug.bGenerateAssertionFile)
	{
		if (!GSettings.Debug.bGenerateAssertionFile)
		{
			AssertionFile << "#if defined(__clang__) || defined(__GNUC__)\n"
			                 "#pragma GCC diagnostic push\n"
			                 "#pragma GCC diagnostic ignored \"-Winvalid-offsetof\"\n"
			                 "#endif\n";
		}
...
void CppGenerator::GenerateEnum(const EnumWrapper& Enum, StreamType& StructFile)
{
	...
	const uint8_t TypeSize           = Enum.GetUnderlyingTypeSize();
	const bool bIsSigned             = Enum.IsUnderlyingTypeSigned();
	const uint8_t TypeBits           = TypeSize * 8;

	uint64_t UTypeMax;
	int64_t STypeMin, STypeMax;
	...
	{
		UTypeMax = (1ULL << TypeBits) - 1;
		STypeMax = (1LL << (TypeBits - 1)) - 1;
		STypeMin = -(1LL << (TypeBits - 1));
	}
	...
	{
		const uint64_t UVal = Info.GetValue();
		const int64_t SVal  = static_cast<int64_t>(UVal);

		const bool bOutOfRange = bIsSigned ? (SVal < STypeMin || SVal > STypeMax) : (UVal > UTypeMax);

		NumValues++;
		MemberString += MakeEnumMemberString(Info.GetUniqueName(), bIsSigned ? std::to_string(SVal) : std::to_string(UVal), UnderlyingType, bOutOfRange);
	}
...
std::string CppGenerator::GetStructPrefixedName(const StructWrapper& Struct)
{
	...
	{
		const UEStruct Owner = Struct.GetUnrealStruct().GetOuter().Cast<UEStruct>();
		if (Owner)
		{
			StructWrapper OwnerWrapper(Owner);
			auto [ValidName, bIsUnique] = OwnerWrapper.GetUniqueName();
			if (bIsUnique)
				return ValidName + "_" + Struct.GetName();

			return PackageManager::GetName(Owner.GetPackageIndex()) + "__" + ValidName + "_" + Struct.GetName();
		}
...
std::string CppGenerator::GetEnumForcedSizeType(const EnumWrapper& Enum, const uint8_t PropertySize)
{
	...
	if (Index > 7)
		return fmt::format("T1ByteEnum<{}>", GetEnumPrefixedName(Enum));

	return fmt::format("{}<{}>", UnderlayingTypesBySize[Index], GetEnumPrefixedName(Enum));
}
...
std::string CppGenerator::GetAssertionMacroString(const std::string& PrefixedStructUniqueName)
{
	...
	return GSettings.Debug.AssertionMacroPrefix + MacroStructName;
}
...
void CppGenerator::GenerateDebugAssertions(StreamType& AssertionStream)
{
	...
	DependencyManager::OnVisitCallbackType GenerateParamStructAssertionsCallback = [&AssertionStream, GenerateAssertionsForStruct](int32 ClassIndex) -> void
	{
		...
		for (const FunctionWrapper& Func : Members.IterateFunctions())
		{
			...
			if (!Func.IsPredefined() && Func.GetParamStructSize() > 0x0)
				GenerateAssertionsForStruct(AssertionStream, Func.AsStruct(), GetStructPrefixedName(Func.AsStruct()));
		}
	};
	...
	for (PackageInfoHandle Package : PackageManager::IterateOverPackageInfos())
	{
		...
		if (Package.HasStructs())
		{
			...
			AssertionStream << fmt::format("\n#define {}_STRUCTS_{} \\\n", GSettings.Debug.AssertionMacroPrefix, PackageName);
...
void CppGenerator::GenerateSDKHeader(StreamType& SdkHpp)
{
	...
	if (!GSettings.CppGenerator.bIncludeParameterStructsInIDA)
	{
	...
	auto ForEachElementCallbackForParams = [&SdkHpp](const PackageManagerIterationParams& OldParams, const PackageManagerIterationParams& NewParams, bool bIsStruct) -> void
	{
		...
		if (!bIsStruct && bHasParameterStructs)
			SdkHpp << fmt::format("#include \"SDK/{}_parameters.hpp\"\n", CurrentPackage.GetName());
	};
...
void CppGenerator::WriteFileHead(StreamType& File, PackageInfoHandle Package, EFileType Type, const std::string& CustomFileComment, const std::string& CustomIncludes)
{
	...
	if (Type != EFileType::BasicCpp && Type != EFileType::Functions)
		File << "#pragma once\n";
...
*/
	...
	if (Type == EFileType::SdkHpp)
		File << fmt::format("\n// {}\n// {}\n", InternalSettings::GameName, InternalSettings::GameVersion);
	...
	if ((Type == EFileType::Functions || Type == EFileType::BasicCpp) && !GSettings.CppGenerator.PrecompiledHeaderFileName.empty())
	{
		File << fmt::format("#include <{}>\n\n", GSettings.CppGenerator.PrecompiledHeaderFileName);
	}
	...
	if (Type == EFileType::BasicHpp)
	{
		...
		if (!GSettings.CppGenerator.XORStringInclude.empty())
		{
			File << fmt::format("#include \"{}\"\n", GSettings.CppGenerator.XORStringInclude);
		}
	}
	...
	if (!GSettings.CppGenerator.SDKNamespaceName.empty())
	{
		File << "SDK_NAMESPACE_START\n";

		if (Type == EFileType::Parameters && !CppSettings.ParamNamespaceName.empty())
			File << "SDK_PARAM_NAMESPACE_START\n";
	}
	else if (!CppSettings.ParamNamespaceName.empty())
	{
...
void CppGenerator::Generate()
{
	...
	GenerateBasicFiles(BasicHpp, BasicCpp, (GSettings.Debug.bGenerateAssertionFile ? DebugAssertions : BasicHpp));
	...
	for (PackageInfoHandle Package : PackageManager::IterateOverPackageInfos())
	{
		...
		const std::string FileName     = GSettings.CppGenerator.FilePrefix + Package.GetName();
		...
		if (Package.HasClasses())
		{
			...
			if (!ClassesFile.is_open())
				GLogger.FmtWrite(ELogLevel::Error, "Error opening file \"{}_classes.hpp\"\n", FileName);
		...
		if (Package.HasStructs() || Package.HasEnums())
		{
			...
			if (!StructsFile.is_open())
				GLogger.FmtWrite(ELogLevel::Error, "Error opening file \"{}_structs.hpp\"\n", FileName);
		...
		if (Package.HasParameterStructs())
		{
			...
			if (!ParametersFile.is_open())
				GLogger.FmtWrite(ELogLevel::Error, "Error opening file \"{}_parameters.hpp\"\n", FileName);
		...
		if (Package.HasFunctions())
		{
			...
			if (!FunctionsFile.is_open())
				GLogger.FmtWrite(ELogLevel::Error, "Error opening file \"{}_functions.cpp\"\n", FileName);
		...
		if (Package.HasStructs())
		{
			...
			StreamType& FileForAssertions = GSettings.Debug.bGenerateAssertionFile ? DebugAssertions : StructsFile;
		...
		if (Package.HasClasses())
		{
			...
			StreamType& FileForAssertions = GSettings.Debug.bGenerateAssertionFile ? DebugAssertions : ClassesFile;
...
void CppGenerator::InitPredefinedMembers()
{
	...
	UFunctionPredefs.Members.insert(UFunctionPredefs.Members.begin(),
	                                PredefinedMember{
	                                    ...
	                                    .Type              = "using FNativeFuncPtr = void (*)(void* Context, void* TheStack, void* Result)",
	                                    .Name              = "",
	                                    .Offset            = 0x0,
	                                    .Size              = 0x00,
	                                    .ArrayDim          = 0x1,
	...
	if (FInstancedStruct && FInstancedStruct.GetStructSize() >= static_cast<int32>(sizeof(void*) * 2))
	{
		...
		FInstancedStructPredefs.Members =
		    {
		        PredefinedMember{
		            ...
		            .Type              = "UScriptStruct*",
		            .Name              = "ScriptStruct",
		            .Offset            = GOffsets.FInstancedStruct.ScriptStruct,
		            .Size              = sizeof(void*),
		            .ArrayDim          = 0x1,
		        ...
		        PredefinedMember{
		            ...
		            .Type              = "uint8*",
		            .Name              = "StructMemory",
		            .Offset            = GOffsets.FInstancedStruct.StructMemory,
		            .Size              = sizeof(void*),
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(void*),
		            .bIsStatic         = false,
	...
	std::string PropertyTypePtr = InternalSettings::bUseFProperty ? "class FProperty*" : "class UProperty*";
	...
	std::vector<PredefinedMember> PropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "int32",
	            .Name              = "ArrayDim",
	            .Offset            = GOffsets.Property.ArrayDim,
	            .Size              = sizeof(int32),
	            .ArrayDim          = 0x1,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "int32",
	            .Name              = "ElementSize",
	            .Offset            = GOffsets.Property.ElementSize,
	            .Size              = sizeof(int32),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(int32),
	            .bIsStatic         = false,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "uint64",
	            .Name              = "PropertyFlags",
	            .Offset            = GOffsets.Property.PropertyFlags,
	            .Size              = sizeof(uint64),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(uint64),
	            .bIsStatic         = false,
	            .bIsZeroSizeMember = false,
	            .bIsBitField       = false,
	            .BitIndex          = 0xFF},
	        PredefinedMember{
	            ...
	            .Type              = "int32",
	            .Name              = "Offset",
	            .Offset            = GOffsets.Property.Offset_Internal,
	            .Size              = sizeof(int32),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(int32),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> BytePropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "class UEnum*",
	            .Name              = "Enum",
	            .Offset            = GOffsets.ByteProperty.Enum,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> BoolPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "uint8",
	            .Name              = "FieldSize",
	            .Offset            = GOffsets.BoolProperty.Base,
	            .Size              = sizeof(uint8),
	            .ArrayDim          = 0x1,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "uint8",
	            .Name              = "ByteOffset",
	            .Offset            = GOffsets.BoolProperty.Base + 0x1,
	            .Size              = sizeof(uint8),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(uint8),
	            .bIsStatic         = false,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "uint8",
	            .Name              = "ByteMask",
	            .Offset            = GOffsets.BoolProperty.Base + 0x2,
	            .Size              = sizeof(uint8),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(uint8),
	            .bIsStatic         = false,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "uint8",
	            .Name              = "FieldMask",
	            .Offset            = GOffsets.BoolProperty.Base + 0x3,
	            .Size              = sizeof(uint8),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(uint8),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> ObjectPropertyBaseMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "class UClass*",
	            .Name              = "PropertyClass",
	            .Offset            = GOffsets.ObjectProperty.PropertyClass,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> ClassPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "class UClass*",
	            .Name              = "MetaClass",
	            .Offset            = GOffsets.ClassProperty.MetaClass,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> StructPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "class UStruct*",
	            .Name              = "Struct",
	            .Offset            = GOffsets.StructProperty.Struct,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> ArrayPropertyMembers =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = PropertyTypePtr,
	            .Name              = "InnerProperty",
	            .Offset            = GOffsets.ArrayProperty.Inner,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> DelegatePropertyMembers =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = "class UFunction*",
	            .Name              = "SignatureFunction",
	            .Offset            = GOffsets.DelegateProperty.SignatureFunction,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> MapPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = PropertyTypePtr,
	            .Name              = "KeyProperty",
	            .Offset            = GOffsets.MapProperty.Base,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = PropertyTypePtr,
	            .Name              = "ValueProperty",
	            .Offset            = GOffsets.MapProperty.Base + (int)sizeof(void*),
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> SetPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Offset            = GOffsets.SetProperty.ElementProp,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> EnumPropertyMembers =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = PropertyTypePtr,
	            .Name              = "UnderlayingProperty",
	            .Offset            = GOffsets.EnumProperty.Base,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	        ...
	        PredefinedMember{
	            ...
	            .Type              = "class UEnum*",
	            .Name              = "Enum",
	            .Offset            = GOffsets.EnumProperty.Base + (int)sizeof(void*),
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> FieldPathPropertyMembers =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = "class FFieldClass*",
	            .Name              = "FieldClass",
	            .Offset            = GOffsets.FieldPathProperty.FieldClass,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	std::vector<PredefinedMember> OptionalPropertyMembers =
	    {
	        PredefinedMember{
	            ...
	            .Type              = PropertyTypePtr,
	            .Name              = "ValueProperty",
	            .Offset            = GOffsets.OptionalProperty.ValueProperty,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(void*),
	            .bIsStatic         = false,
	...
	else
	{
		...
		PredefinedStruct& FProperty           = PredefinedStructs.emplace_back(PredefinedStruct{
		              .UniqueName = "FProperty", .Size = GOffsets.Property.SizeOf, .Alignment = alignof(void*), .bUseExplictAlignment = false, .bIsFinal = false, .bIsClass = true, .Super = &FField /* FField */
        });
		...
		FFieldClass.Properties =
		    {
		        PredefinedMember{
		            ...
		            .Type              = "FName",
		            .Name              = "Name",
		            .Offset            = GOffsets.FFieldClass.Name,
		        ...
		        PredefinedMember{
		            ...
		            .Type              = "uint64",
		            .Name              = "CastFlags",
		            .Offset            = GOffsets.FFieldClass.CastFlags,
		            .Size              = sizeof(uint64),
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(uint64),
		            .bIsStatic         = false,
		...
		const int32 FFieldVariantSize     = InternalSettings::bUseMaskForFieldOwner ? sizeof(void*) : sizeof(void*) * 2;
		const int32 IdentifierOffset      = InternalSettings::bUseMaskForFieldOwner ? 0x0 : sizeof(void*);
		std::string UObjectIdentifierType = (InternalSettings::bUseMaskForFieldOwner ? "constexpr uint64" : "bool");
		std::string UObjectIdentifierName = (InternalSettings::bUseMaskForFieldOwner ? "UObjectMask = 0x1" : "bIsUObject");

		FFieldVariant.Properties =
		    {
		        PredefinedMember{
		            .Comment           = "NOT AUTO-GENERATED PROPERTY",
		            .Type              = "using ContainerType = union { class FField* Field; class UObject* Object; }",
		            .Name              = "",
		            .Offset            = 0x0,
		            .Size              = sizeof(void*),
		            .ArrayDim          = 0x1,
		        ...
		        PredefinedMember{
		            .Comment           = "NOT AUTO-GENERATED PROPERTY",
		            .Type              = UObjectIdentifierType,
		            .Name              = UObjectIdentifierName,
		            .Offset            = IdentifierOffset,
		            .Size              = 0x01,
		            .ArrayDim          = 0x1,
		            .Alignment         = 0x8,
		            .bIsStatic         = InternalSettings::bUseMaskForFieldOwner,
		            .bIsZeroSizeMember = false,
		            .bIsBitField       = false,
		            .BitIndex          = 0xFF},
		    };
		...
		FField.Properties =
		    {
		        ...
		        PredefinedMember{
		            ...
		            .Type              = "FFieldVariant",
		            .Name              = "Owner",
		            .Offset            = GOffsets.FField.Owner,
		            .Size              = FFieldVariantSize,
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(void*),
		            .bIsStatic         = false,
		            .bIsZeroSizeMember = false,
		...
		// Insert FField ObjFlags
		{
			SortMembers(FField.Properties);
			{
				int ObjFlags = FindFreeOffset(FField.Properties, sizeof(int32), sizeof(int32));
				FField.Properties.push_back(PredefinedMember{
				    .Comment           = "NOT AUTO-GENERATED PROPERTY",
				    .Type              = "int32",
				    .Name              = "ObjFlags",
				    .Offset            = ObjFlags,
...
void CppGenerator::InitPredefinedFunctions()
{
	...
	UObjectPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType                    = "class UObject*",
	            .NameWithParams                = "FindObjectImpl(const std::string& FullName, EClassCastFlags RequiredType = EClassCastFlags::None)",
	            .NameWithParamsWithoutDefaults = "FindObjectImpl(const std::string& FullName, EClassCastFlags RequiredType)",
	            .Body =
	                R"({
	...
	return nullptr;
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType                    = "class UObject*",
	            .NameWithParams                = "FindObjectFastImpl(const std::string& Name, EClassCastFlags RequiredType = EClassCastFlags::None)",
	            .NameWithParamsWithoutDefaults = "FindObjectFastImpl(const std::string& Name, EClassCastFlags RequiredType)",
	            .Body =
	                R"({
	for (int i = 0; i < InSDKUtils::GetGObjects()->Num(); ++i)
	{
	...
	return nullptr;
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType         = "UEType*",
	            .NameWithParams     = "FindObject(const std::string& Name, EClassCastFlags RequiredType = EClassCastFlags::None)",
	            .Body =
	                R"({
	return static_cast<UEType*>(FindObjectImpl(Name, RequiredType));
	        ...
	        PredefinedFunction{
	            ...
	            .CustomTemplateText = "template<typename UEType = UObject>",
	            .ReturnType         = "UEType*",
	            .NameWithParams     = "FindObjectFast(const std::string& Name, EClassCastFlags RequiredType = EClassCastFlags::None)",
	            .Body =
	                R"({
	return static_cast<UEType*>(FindObjectFastImpl(Name, RequiredType));
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "class UClass*",
	            .NameWithParams = "FindClass(const std::string& ClassFullName)",
	            .Body =
	                R"({
	return FindObject<class UClass>(ClassFullName, EClassCastFlags::Class);
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "class UClass*",
	            .NameWithParams = "FindClassFast(const std::string& ClassName)",
	            .Body =
	                R"({
	...
	return "None";
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "IsA(const class UClass* TypeClass)",
	            .Body =
	                R"({
	return Class->IsSubclassOf(TypeClass);
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "IsA(const class FName& ClassName)",
	            .Body =
	                R"({
	return Class->IsSubclassOf(ClassName);
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "IsA(EClassCastFlags TypeFlags)",
	            .Body =
	                R"({
	return (Class->CastFlags & TypeFlags);
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "HasTypeFlag(EClassCastFlags TypeFlags)",
	            .Body =
	                R"({
	return (Class->CastFlags & TypeFlags);
})",
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "IsDefaultObject()",
	            .Body =
	                R"({
	return (Flags & EObjectFlags::ClassDefaultObject);
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "void",
	            .NameWithParams = "ProcessEvent(class UFunction* Function, void* Parms)",
	            .Body           = R"({
	...
	UStructPredefs.Functions =
	    {
	        PredefinedFunction{
	            ...
	            .ReturnType     = "bool",
	            .NameWithParams = "IsSubclassOf(const UStruct* Base)",
	            .Body           = IsStructOfTypeCode,
	            .bIsStatic      = false,
	            .bIsConst       = true,
	            .bIsBodyInline  = false,
	            .bIsCtor        = false},
	        PredefinedFunction{
	            .CustomComment  = "Checks if this class has a certain base",
	            .ReturnType     = "bool",
	            .NameWithParams = "IsSubclassOf(const FName& BaseClassName)",
	            .Body =
	                R"({
	...
	UClassPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "class UFunction*",
	            .NameWithParams = "GetFunction(const char* ClassName, const char* FuncName)",
	            .Body =
	                R"({
	...
	return nullptr;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "Gets a UFunction from this UClasses' 'Children' list",
	            .ReturnType     = "class UFunction*",
	            .NameWithParams = "GetFunction(const FName& ClassName, const FName& FuncName)",
	            .Body =
	                R"({
	for (const UStruct* Clss = this; Clss; Clss = Clss->SuperStruct)
	{
	...
	return nullptr;
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "class UFunction*",
	            .NameWithParams = "GetFunction(const FName& FuncName)",
	            .Body =
	                R"({
	for (const UStruct* Clss = this; Clss; Clss = Clss->SuperStruct)
	{
	...
	UEnginePredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "class UEngine*",
	            .NameWithParams = "GetEngine()",
	            .Body =
	                R"({
	...
	for (int i = 0; i < InSDKUtils::GetGObjects()->Num(); i++)
	{
		UObject* Obj = InSDKUtils::GetObjectByIndex(i);
	...
	UWorldPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .ReturnType     = "class UWorld*",
	            .NameWithParams = "GetWorld()",
	            .Body =
	                fmt::format(R"({{{}
	...
	return nullptr;
	                            ...
	                            !GSettings.CppGenerator.bForceNoGWorldInSDK ? GetWorldThroughGWorldCode : ""),
	            .bIsStatic     = true,
	            .bIsConst      = false,
	            .bIsBodyInline = false,
	            .bIsCtor       = false},
	...
	FVectorPredefs.Members.push_back(PredefinedMember{
	    PredefinedMember{
	        .Comment           = "NOT AUTO-GENERATED PROPERTY",
	        .Type              = fmt::format("using UnderlayingType = {}", (InternalSettings::bUseLargeWorldCoordinates ? "double" : "float")),
	        .Name              = "",
	        .Offset            = 0x0,
	        .Size              = 0x08,
	        .ArrayDim          = 0x1,
	...
	FVectorPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .NameWithParams = "FVector(UnderlayingType X = 0, UnderlayingType Y = 0, UnderlayingType Z = 0)",
	            .Body =
	                R"(	: X(X), Y(Y), Z(Z)
{
})",
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FVector(const FVector& other)",
	            .Body =
	                R"(	: X(other.X), Y(other.Y), Z(other.Z)
{
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "FVector",
	            .NameWithParams = "operator+(const FVector& Other)",
	            .Body =
	                R"({
	...
	return { X - Other.X, Y - Other.Y, Z - Other.Z };
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "FVector",
	            .NameWithParams = "operator*(UnderlayingType Scalar)",
	            .Body =
	                R"({
	...
	*this = *this / Other;
	...
	return X == 0 && Y == 0 && Z == 0;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "Dot(const FVector& Other)",
	            .Body =
	                R"({
	return (X * Other.X) + (Y * Other.Y) + (Z * Other.Z);
})",
	...
	return *this / Magnitude();
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "GetDistanceTo(const FVector& Other)",
	            .Body =
	                R"({
	...
	return DiffVector.Magnitude();
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "GetDistanceToInMeters(const FVector& Other)",
	            .Body =
	                R"({
	...
	FVector2DPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FVector2D(UnderlayingType X = 0, UnderlayingType Y = 0)",
	            .Body =
	                R"(	: X(X), Y(Y)
{
})",
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FVector2D(const FVector2D& other)",
	            .Body =
	                R"(	: X(other.X), Y(other.Y)
{
	        ...
	        PredefinedFunction{
	            ...
	            .NameWithParams = "operator+(const FVector2D& Other)",
	            .Body =
	                R"({
	...
	*this = *this / Other;
	...
	return X == 0 && Y == 0;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "Dot(const FVector2D& Other)",
	            .Body =
	                R"({
	return (X * Other.X) + (Y * Other.Y);
})",
	...
	return *this / Magnitude();
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "GetDistanceTo(const FVector2D& Other)",
	            .Body =
	                R"({
	...
	FRotatorPredefs.Functions =
	    {
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FRotator(UnderlayingType Pitch = 0, UnderlayingType Yaw = 0, UnderlayingType Roll = 0)",
	            .Body =
	                R"(	: Pitch(Pitch), Yaw(Yaw), Roll(Roll)
{
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FRotator(const FRotator& other)",
	            .Body =
	                R"(	: Pitch(other.Pitch), Yaw(other.Yaw), Roll(other.Roll)
{
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "ClampAxis(UnderlayingType Angle)",
	            .Body =
	                R"({
	...
	return Angle;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "UnderlayingType",
	            .NameWithParams = "NormalizeAxis(UnderlayingType Angle)",
	            .Body =
	                R"({
	...
	return Angle;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "FRotator",
	            .NameWithParams = "operator+(const FRotator& Other)",
	            .Body =
	                R"({
...
#define SDK_PARAM_NAMESPACE_END }}
	                                                    ...
	                                                    GSettings.CppGenerator.SDKNamespaceName,
...
#include <format>
	                                               ...
	                                               (!GSettings.CppGenerator.SDKNamespaceName.empty() ? SDKMacroDefinitions : ""));
...
namespace Offsets
{{
	constexpr uint64 GObjects         = 0x{:08X};
	constexpr uint64 GNames           = 0x{:08X};
	constexpr uint64 GEngine          = 0x{:08X};
	constexpr uint64 GWorld           = 0x{:08X};
	constexpr uint64 ProcessEvent     = 0x{:08X};
...
namespace InSDKUtils
{
	...
	/* Custom 'GetGNames' function */
	BasicHpp << "\tuintptr_t GetGNames();\n\n";

	/* Custom 'GetGObject' function */
	BasicHpp << "\tTUObjectArray* GetGObjects();\n\n";

	/* Custom 'GetNameByIndex' function */
	BasicHpp << "\tstd::string GetNameByIndex(int32 Index);\n\n";

	/* Custom 'GetObjectByIndex' function */
	BasicHpp << "\tUObject* GetObjectByIndex(int32 Index);\n\n";
...
{})",
	                        GSettings.CppGenerator.GetImageBaseFuncBody);

	BasicCpp << "\n";

	/* Custom 'GetGNames' function */
	BasicCpp << fmt::format(R"(uintptr_t InSDKUtils::GetGNames()
{})",
	                        GSettings.CppGenerator.GetGNamesFuncBody);

	BasicCpp << "\n";

	/* Custom 'GetGObject' function */
	BasicCpp << fmt::format(R"(TUObjectArray* InSDKUtils::GetGObjects()
{})",
	                        GSettings.CppGenerator.GetGObjectsBody);

	BasicCpp << "\n";

	/* Custom 'GetNameByIndex' function */
	BasicCpp << fmt::format(R"(std::string InSDKUtils::GetNameByIndex(int32 Index)
{})",
	                        GSettings.CppGenerator.GetNameByIndexFuncBody);

	BasicCpp << "\n";

	/* Custom 'GetObjectByIndex' function */
	BasicCpp << fmt::format(R"(UObject* InSDKUtils::GetObjectByIndex(int32 Index)
{})",
	                        GSettings.CppGenerator.GetObjectByIndexFuncBody);
...
{ \
	...
	if (GLayouts.ObjectsLayout->GetType() == EObjectsType::Array)
	{
		...
		PredefinedStruct FUObjectItem = PredefinedStruct{
		    .UniqueName = "FUObjectItem", .Size = ObjLayout->FUObjectItem.Size, .Alignment = alignof(void*), .bUseExplictAlignment = false, .bIsFinal = true, .bIsClass = false, .bIsUnion = false, .Super = nullptr};
		...
		FUObjectItem.Properties =
		    {
		        PredefinedMember{
		            ...
		            .Type              = "class UObject*",
		            .Name              = "Object",
		            .Offset            = ObjLayout->FUObjectItem.Object,
		            .Size              = sizeof(void*),
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(void*),
		            .bIsStatic         = false,
	...
	PredefinedStruct FName = PredefinedStruct{
	    .UniqueName = "FName", .Size = GOffsets.FName.SizeOf, .Alignment = alignof(int32), .bUseExplictAlignment = false, .bIsFinal = true, .bIsClass = true, .bIsUnion = false, .Super = nullptr};
	...
	if (!InternalSettings::bUseOutlineNumberName)
	{
		FName.Properties.push_back(PredefinedMember{
		    .Comment           = "NOT AUTO-GENERATED PROPERTY",
		    .Type              = (InternalSettings::bUseNamePool ? "uint32" : "int32"),
		    .Name              = "Number",
		    .Offset            = GOffsets.FName.Number,
		    .Size              = sizeof(int32),
		    .ArrayDim          = 0x1,
		    .Alignment         = alignof(int32),
		    .bIsStatic         = false,
	...
	{
		const int32 DisplayIndexOffset = GOffsets.FName.Number == 4 ? 0x8 : 0x4;

		FName.Properties.push_back(PredefinedMember{
		    .Comment           = "NOT AUTO-GENERATED PROPERTY",
		    .Type              = "int32",
		    .Name              = "DisplayIndex",
		    .Offset            = DisplayIndexOffset,
		    .Size              = sizeof(int32),
		    .ArrayDim          = 0x1,
		    .Alignment         = alignof(int32),
		    .bIsStatic         = false,
	...
	FName.Functions =
	    {
	        ...
	        PredefinedFunction{
	            ...
	            .NameWithParams = fmt::format("explicit FName(int32 ComparisonIndex{}{})",
	                                          !InternalSettings::bUseOutlineNumberName ? ", uint32 Number = 0" : "",
	                                          InternalSettings::bUseCasePreservingName ? ", int32 DisplayIndex = 0" : ""),
	            .Body =
	                fmt::format(R"(	: ComparisonIndex(ComparisonIndex){}{}
{{
}})",
	                            !InternalSettings::bUseOutlineNumberName ? ", Number(Number)" : "",
	                            InternalSettings::bUseCasePreservingName ? ", DisplayIndex(DisplayIndex)" : ""),
	            .bIsStatic     = false,
	            .bIsConst      = false,
	            .bIsBodyInline = true,
	            .bIsCtor       = true},
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FName(const FName&) = default;",
	            .Body           = "",
	            .bIsStatic      = false,
	            .bIsConst       = false,
	            .bIsBodyInline  = true,
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr",
	            .NameWithParams = "FName(FName&&) = default;",
	            .Body           = "",
	            .bIsStatic      = false,
	            .bIsConst       = false,
	            .bIsBodyInline  = true,
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr FName&",
	            .NameWithParams = "operator=(const FName&) = default;",
	            .Body           = "",
	            .bIsStatic      = false,
	            .bIsConst       = false,
	            .bIsBodyInline  = true,
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "constexpr  FName&",
	            .NameWithParams = "operator=(FName&&) = default;",
	            .Body           = "",
	            .bIsStatic      = false,
	            .bIsConst       = false,
	            .bIsBodyInline  = true,
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "bool",
	            .NameWithParams = "IsNone()",
	            .Body =
	                fmt::format(R"({{
	return !{}{};
}}
)",
	                            InternalSettings::bUseCasePreservingName ? "DisplayIndex" : "ComparisonIndex",
	                            !InternalSettings::bUseOutlineNumberName ? "&& !Number" : ""),
	            .bIsStatic     = false,
	            .bIsConst      = true,
	            .bIsBodyInline = true,
	            .bIsCtor       = false},
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "int32",
	            .NameWithParams = "GetDisplayIndex()",
	            .Body =
	                fmt::format(R"({{
	return {};
}}
	...
	return OutputString.substr(pos + 1);
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "bool",
	            .NameWithParams = "operator==(const FName& Other)",
	            .Body =
	                fmt::format(R"({{
	return ComparisonIndex == Other.ComparisonIndex{};
}})",
	                            !InternalSettings::bUseOutlineNumberName ? " && Number == Other.Number" : ""),
	            .bIsStatic     = false,
	            .bIsConst      = true,
	            .bIsBodyInline = true,
	            .bIsCtor       = false},
...
public:
		...
		FStructBaseChain.Properties =
		    {
		        PredefinedMember{
		            .Comment           = "NOT AUTO-GENERATED PROPERTY",
		            .Type              = "FStructBaseChain**",
		            .Name              = "StructBaseChainArray",
		            .Offset            = 0x0,
		            .Size              = sizeof(void*),
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(void*),
		        ...
		        PredefinedMember{
		            .Comment           = "NOT AUTO-GENERATED PROPERTY",
		            .Type              = "int32",
		            .Name              = "NumStructBasesInChainMinusOne",
		            .Offset            = sizeof(void*),
		            .Size              = sizeof(int32),
		            .ArrayDim          = 0x1,
		            .Alignment         = alignof(void*),
	...
	const int32 TextDataSize = (GInSDKOffsets.FText.InTextDataString + sizeof(FString));
	...
	FTextData.Properties =
	    {
	        PredefinedMember{
	            ...
	            .Type              = "class FString",
	            .Name              = "TextSource",
	            .Offset            = GInSDKOffsets.FText.InTextDataString,
	            .Size              = sizeof(FString),
	            .ArrayDim          = 0x1,
	            .Alignment         = alignof(FString),
	            .bIsStatic         = false,
	            .bIsZeroSizeMember = false,
	            .bIsBitField       = false,
	            .BitIndex          = 0xFF},
...
{)";
	...
	PredefinedStruct FText = PredefinedStruct{
	    .UniqueName = "FText", .Size = GInSDKOffsets.FText.Size, .Alignment = sizeof(void*), .bUseExplictAlignment = false, .bIsFinal = true, .bIsClass = true, .bIsUnion = false, .Super = nullptr};

	FText.Properties =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = "class FTextImpl::FTextData*",
	            .Name              = "TextData",
	            .Offset            = GInSDKOffsets.FText.TextData,
	            .Size              = sizeof(void*),
	            .ArrayDim          = 0x1,
	            .Alignment         = 0x1,
	            .bIsStatic         = false,
	...
	FText.Functions =
	    {
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "const class FString&",
	            .NameWithParams = "GetStringRef()",
	            .Body =
	                R"({
	...
	if (!IsValid())
		return nullptr;
	return InSDKUtils::GetObjectByIndex(ObjectIndex);
})",
	            .bIsStatic     = false,
	            .bIsConst      = true,
	            .bIsBodyInline = false,
	...
	return InSDKUtils::GetObjectByIndex(ObjectIndex);
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "bool",
	            .NameWithParams = "operator==(const FWeakObjectPtr& Other)",
	            .Body =
	                R"({
	...
	return ObjectIndex != Other.ObjectIndex;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "bool",
	            .NameWithParams = "operator==(const class UObject* Other)",
	            .Body =
	                R"({
...
public:
	...
	PredefinedStruct TPersistentObjectPtr = PredefinedStruct{
	    ...
	    .UniqueName           = "TPersistentObjectPtr",
	    .Size                 = 0x0,
	    .Alignment            = sizeof(void*),
	    .bUseExplictAlignment = false,
	    .bIsFinal             = false,
	    .bIsClass             = true,
	    .bIsUnion             = false,
	    .Super                = nullptr};

	const int32 ObjectIDOffset = InternalSettings::bIsWeakObjectPtrWithoutTag ? 0x8 : 0xC;
	...
	if (InternalSettings::bIsWeakObjectPtrWithoutTag)
...
public:
	...
	if (GSettings.EngineCore.bEnableEncryptedObjectPropertySupport)
	{
...
public:
	...
	return ObjectPointer;
	        ...
	        PredefinedFunction{
	            .CustomComment  = "",
	            .ReturnType     = "void*",
	            .NameWithParams = "GetInterfaceRef()",
	            .Body =
	                R"({
	...
	{
		// class FFieldPath
		// Use explict alignment for padding
		PredefinedStruct FFieldPath = PredefinedStruct{
		    .UniqueName = "FFieldPath", .Size = GInSDKOffsets.FieldPathProperty.SizeOf, .Alignment = alignof(void*), .bUseExplictAlignment = true, .bIsFinal = false, .bIsClass = true, .bIsUnion = false, .Super = nullptr};
		...
		PredefinedStruct TFieldPath = PredefinedStruct{
		    ...
		    .UniqueName           = "TFieldPath",
		    .Size                 = GInSDKOffsets.FieldPathProperty.SizeOf,
		    .Alignment            = alignof(void*),
		    .bUseExplictAlignment = false,
		    .bIsFinal             = true,
		    .bIsClass             = true,
...
public:
	...
	} /* End 'if (InternalSettings::bUseFProperty)' */

	const auto ScriptDelegateSize = (FWeakObjectPtrSize + GOffsets.FName.SizeOf);
	...
	PredefinedStruct TDelegate = PredefinedStruct{
	    ...
	    .UniqueName           = "TDelegate",
	    .Size                 = GInSDKOffsets.DelegateProperty.SizeOf,
	    .Alignment            = 0x4,
	    .bUseExplictAlignment = false,
	    .bIsFinal             = false,
	    .bIsClass             = true,
	...
	PredefinedStruct TMulticastInlineDelegate = PredefinedStruct{
	    .CustomTemplateText   = "template<typename FunctionSignature>",
	    .UniqueName           = "TMulticastInlineDelegate",
	    .Size                 = GInSDKOffsets.MulticastInlineDelegateProperty.SizeOf,
	...
	PredefinedStruct TMulticastInlineDelegateSpezialiation = PredefinedStruct{
	    .CustomTemplateText   = "template<typename Ret, typename... Args>",
	    .UniqueName           = "TMulticastInlineDelegate<Ret(Args...)>",
	    .Size                 = 0x0,
	    .Alignment            = 0x1,
	    .bUseExplictAlignment = false,
	    .bIsFinal             = false,
	...
	TMulticastInlineDelegateSpezialiation.Properties =
	    {
	        PredefinedMember{
	            .Comment           = "NOT AUTO-GENERATED PROPERTY",
	            .Type              = "TArray<FScriptDelegate>",
	            .Name              = "InvocationList",
	            .Offset            = static_cast<int32>(GInSDKOffsets.MulticastInlineDelegateProperty.SizeOf - sizeof(TArray<int>)),
...
using TActorBasedCycleFixup = CyclicDependencyFixupImpl::TCyclicClassFixup<UnderlayingClassType, Size, Align, class AActor>;
	...
	// std::formatter specialisation for the generated SDK was dropped: it adds an iOS 16.3+
	// requirement the rest of the SDK doesn't have.
}
...
{
	WriteFileHead(UEContainersHeader, nullptr, EFileType::UnrealContainers, "Container implementations with iterators. See https://github.com/Fischsalat/UnrealContainers", "#include <string>\n#include <stdexcept>\n#include <iostream>\n#include <optional>\n#include \"UtfN.hpp\"");
...
#endif // IMPORT_CPP_SDK_INTO_IDA
{
	...
	UEContainersHeader << (InternalSettings::bUseChar16String ? "\ttypedef char16_t TCHAR;\n" : "\ttypedef wchar_t TCHAR;\n");
	UEContainersHeader << R"(
	typedef int8_t  int8;
...
{
	WriteFileHead(UnicodeLib, nullptr, EFileType::UnicodeLib, "A simple C++ lib for converting between Utf8, Utf16 and Utf32. See https://github.com/Fischsalat/UTF-N");
